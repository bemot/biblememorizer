...
function filterData(cur_bible) {
  var index = 1;
  var b = {};
  if (cur_bible.name === "King James") {
    index = 0;
  }
  if (cur_bible.name === "Ukrainian Ogienko") {
    index = 1;
  }
  if (cur_bible.name === "Russian Synodal") {
    index = 2;
  }
  b = data.bibles[index];
...
function ActiveBook(props) {
  ...
  for (var i = 0; i < props.bible.books.length; i++) {
    knygy.push({ book_id: i, book_name: props.bible.books[i].name });
  }
...
function ActiveVerse(props) {
  ...
  for (
    var i = 0;
    i <
    props.bible.books[props.book_num].chapters[props.chap_num].verses.length;
    i++
  ) {
    virshi.push({
      verse_id: i + 1,
      verse_name:
        props.bible.books[props.book_num].chapters[props.chap_num].verses[i]
          .name,
    });
  }
